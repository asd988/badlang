    ...
    fn execute_instrucion(&mut self, ix: usize) -> bool {
        ...
        match instruction {
             Instruction::Declaration(simple_operation) => {
                self.variables.insert(simple_operation.identifier.name.clone(), self.get_from_value(&simple_operation.value));
            },
            Instruction::Add(simple_operation) => {
                let value = self.get_from_value(&simple_operation.value);
                let variable = self.variables.get_mut(&simple_operation.identifier.name).unwrap();
                *variable += value;
            },
            ...
            Instruction::Invert(identifier) => {
                let variable = self.variables.get_mut(&identifier.name).unwrap();
                *variable = if *variable == 0 { 1 } else { 0 };
            },
            Instruction::Delete(identifier) => {
                self.variables.remove(&identifier.name);
            },
            ...
            Instruction::Jump(tag, value) => {
                let tag = self.code.tags.get(&tag.name).unwrap();
